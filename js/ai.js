...
(function () {
  ...
  const baziLocal = {
    ...
    chat(q, p) {
      ...
      if (/财/.test(kw)) return '就财运而言，你命局五行分布为：' + wxStr + '。' + (missing.length ? '五行缺 ' + missing.join('、') + '，宜补足调候以旺财库。' : '五行俱全，财源较广。') + '近期正财为主，宜踏实积累、理性规划，切忌冲动消费或投机冒进。开源节流双管齐下，财运自会稳步增长。';
      if (/工作|事业|职业|上班/.test(kw)) return '就事业而言，你命主日干属' + ((p && p.rizhuWx) || '') + '性，宜顺五行之旺势择业：' + (missing.length ? '补足所缺，扬长避短。' : '天资周正，选择面广。') + '建议聚焦一个方向深耕，把长板做到极致，贵人自会被你的专注吸引而来。';
      if (/感情|婚姻|姻缘|对象|恋爱|婚/.test(kw)) return '就感情而言，命局讲究阴阳调和、相生相济。' + (p && p.sex === '男' ? '男命以财星为妻（财星，可理解为吸引你、让你愿意付出的人缘与家室），日主得养则姻缘和美。' : '女命以官星为夫（官星，可理解为与你情投意合、相互扶持的另一半），性情温润则家庭和睦。') + '感情里真诚比技巧重要，多沟通、多体谅，顺其自然，良缘自会靠近。';
      if (/健康|身体|病|睡/.test(kw)) return '就健康而言，' + (missing.length ? '你五行缺 ' + missing.join('、') + '，对应脏腑宜多加养护，规律作息、适度运动为要。' : '你五行均衡，体魄底子不错，注意劳逸结合、情绪疏解即可。') + '身体是最大的本钱，别拿健康换效率。';
      if (/转运|运气|流年|明年|今年/.test(kw)) return '就运势而言，流年更替、五行流转是常理。把握当下能把握的，保持谦逊与努力，转运之年自会到来。建议今年多积累、少折腾，静待时机成熟再发力。';
      if (/[0-9]{4}年/.test(kw)) return '年份运势受流年干支与大运互动影响。建议在该年多行正道、广结善缘，顺势而为，遇事多听取长辈或专业意见，自能趋吉避凶。';
