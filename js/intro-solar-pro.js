...
(function () {
  ...
  var FRAG_PLANET = [
    ...
    'float fbm(vec2 p){ float v=0.0, a=0.5; for(int i=0;i<5;i++){ v+=a*noise(p); p=p*2.17+vec2(1.9,3.3); a*=0.5; } return v; }',
    'void main(){',
    ...
    '  vec2 p = vec2(lon, lat);',
    '',
    '  /* 高级纹理：多层噪声，细腻条纹 + 斑块 + 涡旋 */',
    '  float band1 = fbm(vec2(lon*4.0, lat*8.0));',
    '  float band2 = fbm(vec2(lon*9.0 + 5.0, lat*14.0));',
    '  float vortex = fbm(vec2(lon*12.0 - lat*6.0, lat*10.0 + lon*4.0) + uTime*0.02);',
    '  float stripe = 0.5 + 0.5 * sin(lat*13.0 + band1*4.0 + band2*3.0 + vortex*2.0);',
    '  float detail = fbm(vec2(lon*15.0, lat*20.0) + 9.0);',
    '',
    '  vec3 col;',
    '  if (uType < 0.5) {',
    '    /* 气态巨行星：细腻多层条纹 */',
    '    float s = smoothstep(0.1, 0.9, stripe);',
    '    col = mix(uColorA, uColorB, s);',
    '    col = mix(col, uColorB, detail * 0.2);',
    '    /* 大红斑/涡旋 */',
    '    float spot = smoothstep(0.75, 0.95, vortex);',
    '    col = mix(col, mix(uColorA, vec3(0.9, 0.5, 0.4), 0.5), spot * 0.5);',
    '  } else if (uType < 1.5) {',
    '    /* 岩石行星：大陆 + 海洋 + 表面细节 */',
    '    float land = smoothstep(0.42, 0.66, fbm(p*2.5+2.0));',
    '    float mountain = smoothstep(0.7, 0.92, fbm(p*6.0+4.0));',
    '    col = mix(uColorA, uColorB, land);',
    '    col = mix(col, vec3(0.85, 0.8, 0.7), mountain * 0.4);',
    '    col *= (0.95 + 0.05 * fbm(p*10.0));',
    '  } else if (uType < 2.5) {',
    '    /* 冰巨星：光滑 + 极冠 + 云带 */',
    '    float ice = smoothstep(0.55, 0.92, abs(lat));',
    '    float cloud = smoothstep(0.6, 0.85, band1 * 0.6 + band2 * 0.4);',
    '    col = mix(uColorA, uColorB, cloud);',
    '    col = mix(col, vec3(0.96, 0.98, 1.0), ice * 0.95);',
    '  } else {',
    '    /* 暖岩：条纹 + 细节 */',
    '    col = mix(uColorA, uColorB, smoothstep(0.2, 0.8, stripe*0.5 + detail*0.6));',
    '    col *= (0.95 + 0.05 * fbm(p*8.0));',
    '  }',
    '',
    '  /* 光照：环境 + 漫反射 + 柔和高光 */',
    ...
    '  col += vec3(1.0, 0.98, 0.92) * pow(max(dot(n, hv), 0.0), 26.0) * diff * 0.35;',
    '',
    '  /* 大气光晕：受光面暖金、背光面蓝紫，高级感 */',
    '  float fres = pow(1.0 - max(dot(n, vec3(0.0,0.0,1.0)), 0.0), 2.4);',
    '  vec3 atmo = mix(vec3(0.3, 0.5, 0.9), vec3(0.95, 0.75, 0.5), diff);',
    '  col += atmo * fres * 0.6;',
    '',
